...
@data InferenceSignalVariantDataType begin
...
@derive InferenceSignalVariantDataType[Hash, Eq, Show]

const InferenceSignalVariant = InferenceSignalVariantDataType.Type
...
The `InferenceSignal` variants are defined in [`InferenceSignalVariantDataType`](@ref).
"""
...
See also [`InferenceSignalVariantDataType`](@ref) for the possible variants.
"""
function create_inference_signal()::InferenceSignal
    return Signal(Any, InferenceSignalVariant, UndefValue(), InferenceSignalVariantDataType.Unspecified())
end
