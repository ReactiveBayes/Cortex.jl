...
for n in [10, 100, 1000, 10_000, 100_000]
    ...
    DualPendingBenchmarks["check all pending when none set", n] = @benchmarkable begin
        Cortex.is_pending_in_all(dpg)
    end setup = begin
        dpg = Cortex.DualPendingGroup($n)
    end

    DualPendingBenchmarks["check all pending when half set", n] = @benchmarkable begin
        Cortex.is_pending_in_all(dpg)
    end setup = begin
        dpg = Cortex.DualPendingGroup($n)
        foreach(1:($n ÷ 2)) do i
    ...
    DualPendingBenchmarks["check all pending all but one set", n] = @benchmarkable begin
        Cortex.is_pending_in_all(dpg)
    end setup = begin
        dpg = Cortex.DualPendingGroup($n)
        foreach(1:($n - 1)) do i
    ...
    DualPendingBenchmarks["check all pending when all set", n] = @benchmarkable begin
        Cortex.is_pending_in_all(dpg)
    end setup = begin
        dpg = Cortex.DualPendingGroup($n)
        foreach(1:($n)) do i
